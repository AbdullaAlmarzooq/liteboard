...
const EditTicket = () => {
    ...
    if (formData.workflowId && workflows) {
      ...
      if (selectedWorkflow && selectedWorkflow.steps) {
        ...
        const currentStep = allSteps.find(
          step => step.stepName === formData.status || step.stepCode === formData.status
        );
  ...
  // --- Comment handlers (using /api/comments) ---
    ...
    try {
      ...
      if (res.ok) {
        ...
        setComments([...comments, { 
          id: created.id, text: newCommentText, author: 'Current User', timestamp: new Date().toISOString() 
        }]);
        setNewCommentText('');
      } else setSubmitError('Failed to add comment.');
    } catch (err) {
  ...
  const handleSaveCommentEdit = async (commentId) => {
    ...
    try {
      ...
      if (res.ok) {
        setComments(comments.map(c => c.id === commentId ? { ...c, text: editingCommentText } : c));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } else setSubmitError('Failed to update ticket.');
    } catch {
  ...
  if (isPending || workflowsPending) {
    return <div className="flex items-center justify-center min-h-64"><div className="text-gray-500">Loading ticket...</div></div>;
  }
  if (error) {
    return <div className="flex items-center justify-center min-h-64"><div className="text-red-500">Error loading ticket: {error}</div></div>;
  }
  if (!ticket) {
    return <div className="flex items-center justify-center min-h-64"><div className="text-gray-500">Ticket not found</div></div>;
  }
    ...
    <div className="relative space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <button onClick={() => navigate("/tickets")} className="text-blue-600 hover:text-blue-800 text-sm font-medium">
              ← Back to Tickets
            </button>
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Ticket</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <CommentSection
              ...
              handleDeleteComment={setCommentToDeleteId}
              handleSaveCommentEdit={handleSaveCommentEdit}
              handleCancelCommentEdit={() => { setEditingCommentId(null); setEditingCommentText(''); }}
                    ...
                    return (
                      <li key={file.id} className="flex items-center justify-between py-2">
                        <div className="flex items-center gap-3">
                          {isImage && <img src={file.data} alt="Attachment" className="w-10 h-10 object-cover rounded-md" />}
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          <a href={file.data} download={file.name} className="text-blue-600 hover:text-blue-800 text-sm font-medium">Download</a>
                          <button type="button" onClick={() => handleRemoveSavedAttachment(file)} className="text-red-600 hover:text-red-800 text-sm font-medium">Remove</button>
                        </div>
                      </li>
                    );
                  })}
        ...
        <div className="flex gap-4 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          <Button type="button" variant="outline" onClick={() => navigate(`/view-ticket/${ticket.id}`)}>
            Cancel
          </Button>
        </div>
      </form>
