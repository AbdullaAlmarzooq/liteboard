...
const CreateTicketPage = () => {
  ...
  const {
    ...
    error: projectsError,
  } = useFetch(apiUrl("/api/projects/available"))
  const {
    data: employees,
    isPending: employeesLoading,
    error: employeesError,
  } = useFetch(apiUrl("/api/employees"))
  const selectedProjectId = formData.projectId
  const modulesUrl = selectedProjectId
    ? apiUrl(`/api/modules?project_id=${encodeURIComponent(selectedProjectId)}`)
    : null
  const {
    data: modules,
  ...
  const workflowsUrl = selectedProjectId
    ? apiUrl(`/api/workflows?project_id=${encodeURIComponent(selectedProjectId)}`)
    : null
  const tagsUrl = selectedProjectId
    ? apiUrl(`/api/tags?project_id=${encodeURIComponent(selectedProjectId)}`)
    : null
  ...
  const generateTicketId = async () => {
    try {
      const response = await fetchWithAuth(apiUrl("/api/tickets"))
      if (!response.ok) {
