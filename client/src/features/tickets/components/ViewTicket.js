...
  const { data: rawTicket, isPending, error } = useFetch(apiUrl(`/api/tickets/${ticketId}?include_blobs=false`))
  
  const { data: ticketEvents, isPending: isEventsPending } = useFetch(apiUrl(`/api/tickets/${ticketId}/events`))
...
      fetchWithAuth(apiUrl(`/api/workflows/${ticket.workflowId}`))
        .then(res => {
