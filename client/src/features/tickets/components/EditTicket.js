import { useFetch, fetchWithAuth, apiUrl } from "../../../lib/api";
// EditTicket.js
...
const EditTicket = () => {
  ...
  const { user } = useAuth();
  const { data: ticket, isPending, error } = useFetch(apiUrl(`/api/tickets/${ticketId}?include_blobs=false`));
  const { data: employees } = useFetch(apiUrl("/api/employees")); 
  const ticketProjectId = ticket?.project_id || ticket?.projectId || '';
  const tagsUrl = ticketProjectId
    ? apiUrl(`/api/tags?project_id=${encodeURIComponent(ticketProjectId)}`)
    : null;
  const modulesUrl = ticketProjectId
    ? apiUrl(`/api/modules?project_id=${encodeURIComponent(ticketProjectId)}`)
    : null;
  const { data: tagsList } = useFetch(tagsUrl); 
  const { data: workgroups } = useFetch(apiUrl("/api/workgroups"));
  const { data: workflows, isPending: workflowsPending } = useFetch(apiUrl("/api/workflows"));
  ...
  useEffect(() => {
    const fetchAllowedSteps = async () => {
      ...
      try {
        ...
        const response = await fetch(apiUrl(`/api/tickets/${ticketId}/allowed-steps`), {
          method: "GET",
          headers: {
  ...
  const fetchAttachmentBlob = async (attachmentId) => {
    ...
    const res = await fetchWithAuth(apiUrl(`/api/attachments/${attachmentId}/blob`));
  ...
  const uploadNewAttachments = async () => {
    ...
    for (const attachment of newAttachments) {
      const response = await fetchWithAuth(apiUrl("/api/attachments"), {
        method: "POST",
        body: JSON.stringify({
          ticket_id: ticketRef,
  ...
  const deleteRemovedAttachments = async () => {
    ...
    for (const attachmentId of removedAttachmentIds) {
      const response = await fetchWithAuth(apiUrl(`/api/attachments/${attachmentId}`), {
        method: "DELETE",
      });
  ...
  const handleCommentSubmit = async () => {
    ...
    setSubmitError('');
    try {
      const res = await fetchWithAuth(apiUrl(`/api/comments`), {
        method: 'POST',
        body: JSON.stringify({
          ticket_id: ticketId,
  ...
  const confirmDeleteComment = async () => {
    ...
    try {
      const res = await fetchWithAuth(apiUrl(`/api/comments/${commentToDeleteId}`), {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  if (
    ...
    try {
      ...
      if (hasStatusChanged) {
        const transitionResponse = await fetchWithAuth(apiUrl(`/api/tickets/${ticketId}/transition`), {
          method: 'POST',
      ...
      const res = await fetchWithAuth(apiUrl(`/api/tickets/${ticketId}`), {
        method: 'PUT',
