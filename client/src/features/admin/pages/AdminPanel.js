...
const WORKFLOW_LIST_ENDPOINT = apiUrl("/api/workflow_management/list");
const PROJECT_LIST_ENDPOINT = apiUrl("/api/projects/list");
...
const AdminPanel = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [empRes, tagsRes, wgRes, modRes, wfRes, projectsRes, rolesRes] = await Promise.all([
        fetchWithAuth(apiUrl("/api/employees")),
        fetchWithAuth(apiUrl("/api/tags")),
        fetchWithAuth(apiUrl("/api/workgroups")),
        fetchWithAuth(apiUrl("/api/modules")),
        fetchWithAuth(WORKFLOW_LIST_ENDPOINT),
        fetchWithAuth(PROJECT_LIST_ENDPOINT),
        fetchWithAuth(apiUrl("/api/employees/roles"))
      ]);
  ...
  const saveWorkflow = async (wf) => {
    try {
      ...
      if (!wf.id) {
        ...
        const res = await fetchWithAuth(apiUrl("/api/workflow_management"), {
          method: 'POST',
        ...
        const res = await fetchWithAuth(apiUrl(`/api/workflow_management/${wf.id}`), {
          method: 'PATCH',
  ...
  const handleSave = async () => {
    try {
      if (activeTab === 'tags') {
        ...
        const response = await fetchWithAuth(apiUrl(`/api/tags/${editForm.id}`), {
          method: 'PUT',
          body: JSON.stringify(payload)
        });
      ...
      if (activeTab === 'modules') {
        ...
        const response = await fetchWithAuth(apiUrl(`/api/modules/${editForm.id}`), {
          method: 'PUT',
          body: JSON.stringify(payload),
        });
      ...
      if (activeTab === 'workgroups') {
        ...
        const response = await fetchWithAuth(apiUrl(`/api/workgroups/${editForm.id}`), {
          method: 'PUT',
          body: JSON.stringify(payload),
        });
      ...
      const response = await fetchWithAuth(apiUrl(`/api/employees/${editForm.id}`), {
        method: 'PUT',
        body: JSON.stringify(payload)
      });
      ...
      const updatedEmployeeRes = await fetchWithAuth(apiUrl(`/api/employees/${editForm.id}`));
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetchWithAuth(apiUrl(`/api/tags/${tag.id}`), {
        method: 'DELETE',
      });
  ...
  const handleCreateSave = async () => {
    ...
    try {
      const createEndpoint = apiUrl(`/api/${activeTab}`);
  ...
  const handleProjectEdit = (project) => {
    ...
    fetchWithAuth(apiUrl(`/api/projects/${project.id}`))
      .then(async (response) => {
    ...
    try {
      ...
      if (isEditing) {
        await parseApiResponse(
          await fetchWithAuth(apiUrl(`/api/projects/${projectForm.id}`), {
            method: 'PUT',
            body: JSON.stringify({
        ...
        await parseApiResponse(
          await fetchWithAuth(apiUrl(`/api/projects/${projectForm.id}/workgroups`), {
            method: 'PUT',
            body: JSON.stringify({
        ...
        await parseApiResponse(
          await fetchWithAuth(apiUrl(`/api/projects/${projectForm.id}/workflows`), {
            method: 'PUT',
            body: JSON.stringify({
        ...
        await parseApiResponse(
          await fetchWithAuth(apiUrl(`/api/projects/${projectForm.id}/modules`), {
            method: 'PUT',
            body: JSON.stringify({
        ...
        await parseApiResponse(
          await fetchWithAuth(apiUrl("/api/projects"), {
            method: 'POST',
      ...
      if (isEditing) {
        const refreshedProjectResponse = await fetchWithAuth(apiUrl(`/api/projects/${projectForm.id}`));
  ...
  const confirmProjectToggleActive = async () => {
    ...
    try {
      const response = await fetchWithAuth(apiUrl(`/api/projects/${project.id}`), {
        method: 'PUT',
        body: JSON.stringify({
          name: project.name,
