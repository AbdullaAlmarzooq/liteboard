...
const WorkflowsTab = ({ workflows: initialWorkflows, workgroups, onEdit: parentOnEdit }) => {
  ...
  const handleEditClick = async (workflow) => {
    ...
    try {
      const res = await fetchWithAuth(apiUrl(`/api/workflow_management/${workflow.id}`));
      if (!res.ok) {
  ...
  const confirmToggleActive = async () => {
    ...
    try {
      const res = await fetchWithAuth(apiUrl(`/api/workflow_management/${workflow.id}/active`), {
        method: 'PATCH',
