...
import PendingTicketsPerTypeChart from "../components/PendingTicketsPerTypeChart";
import OpenTicketsPieChart from "../components/PendingTicketsPieChart";
...
const Dashboard = () => {
  ...
  const { 
    ...
    error: ticketsError 
  } = useFetch(apiUrl("/api/tickets"));

  const { 
    data: workgroupsData, 
    isPending: workgroupsPending, 
    error: workgroupsError 
  } = useFetch(apiUrl("/api/workgroups"));
